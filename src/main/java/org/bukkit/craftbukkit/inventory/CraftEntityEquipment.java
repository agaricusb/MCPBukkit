...
public class CraftEntityEquipment implements EntityEquipment {
    ...
    private ItemStack getEquipment(int slot) {
        return CraftItemStack.asBukkitCopy(entity.getHandle().func_71124_b/*was:getEquipment*/(slot));
    }

    private void setEquipment(int slot, ItemStack stack) {
        entity.getHandle().func_70062_b/*was:setEquipment*/(slot, CraftItemStack.asNMSCopy(stack));
    }
    ...
    private void setDropChance(int slot, float chance) {
        entity.getHandle().field_82174_bp/*was:dropChances*/[slot] = chance - 0.1F;
    }

    private float getDropChance(int slot) {
        return entity.getHandle().field_82174_bp/*was:dropChances*/[slot] + 0.1F;
    }
}
