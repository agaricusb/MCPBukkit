...
public abstract class MinecraftServer implements ICommandSender, Runnable, IPlayerUsage
{
    ...
    private void func_82355_al()
    {
        ...
        BlockDispenser.field_82527_a.func_82595_a(Item.field_77773_az, behaviordispenseminecart);
        BlockDispenser.field_82527_a.func_82595_a(Item.field_77762_aN, behaviordispenseminecart);
        BlockDispenser.field_82527_a.func_82595_a(Item.field_77763_aO, behaviordispenseminecart);
        ...
        BlockDispenser.field_82527_a.func_82595_a(Item.field_77775_ay, behaviorbucketfulldispense);
        BlockDispenser.field_82527_a.func_82595_a(Item.field_77786_ax, behaviorbucketfulldispense);
    ...
    protected void func_71247_a(String p_71247_1_, String p_71247_2_, long p_71247_3_, WorldType p_71247_5_, String p_71247_6_)
    {
        ...
        for (int j = 0; j < worldCount; ++j)
        {
            ...
            if (j == 0)
            {
                ...
                {
                    // CraftBukkit
                    world = new DemoWorldServer(this, new AnvilSaveHandler(server.getWorldContainer(), p_71247_2_, true), p_71247_2_, dimension, this.field_71304_b);
                }
                else
                {
                    // CraftBukkit
                    world = new WorldServer(this, new AnvilSaveHandler(server.getWorldContainer(), p_71247_2_, true), p_71247_2_, dimension, worldsettings, this.field_71304_b, Environment.getEnvironment(dimension), gen);
                }
            }
            else
            {
                ...
                {
                    field_71306_a.info("---- Migration of old " + worldType + " folder required ----");
                    field_71306_a.info("Unfortunately due to the way that Minecraft implemented multiworld support in 1.6, Bukkit requires that you move your " + worldType + " folder to a new location in order to operate correctly.");
                    field_71306_a.info("We will move this folder for you, but it will mean that you need to move it back should you wish to stop using Bukkit in the future.");
                    field_71306_a.info("Attempting to move " + oldWorld + " to " + newWorld + "...");

                    if (newWorld.exists())
                    {
                        field_71306_a.severe("A file or folder already exists at " + newWorld + "!");
                        field_71306_a.info("---- Migration of old " + worldType + " folder failed ----");
                    }
                    else if (newWorld.getParentFile().mkdirs())
                    {
                        if (oldWorld.renameTo(newWorld))
                        {
                            field_71306_a.info("Success! To restore " + worldType + " in the future, simply move " + newWorld + " to " + oldWorld);
                            ...
                            {
                                field_71306_a.severe("Unable to migrate world data.");
                            }

                            field_71306_a.info("---- Migration of old " + worldType + " folder complete ----");
                        }
                        else
                        {
                            field_71306_a.severe("Could not move folder " + oldWorld + " to " + newWorld + "!");
                            field_71306_a.info("---- Migration of old " + worldType + " folder failed ----");
                        }
                    }
                    else
                    {
                        field_71306_a.severe("Could not create path for " + newWorld + "!");
                        field_71306_a.info("---- Migration of old " + worldType + " folder failed ----");
                    }
                }
                ...
                // CraftBukkit
                world = new WorldServerMulti(this, new AnvilSaveHandler(server.getWorldContainer(), name, true), name, dimension, worldsettings, this.worlds.get(0), this.field_71304_b, Environment.getEnvironment(dimension), gen);
            }
    ...
    protected void func_71222_d()
    {
        ...
        for (int j = 0; j < this.worlds.size(); ++j)
        {
            ...
            field_71306_a.info("Preparing start region for level " + j + " (Seed: " + worldserver.func_72905_C() + ")");
            ...
            for (int k = -short1; k <= short1 && this.func_71278_l(); k += 16)
            {
                for (int l = -short1; l <= short1 && this.func_71278_l(); l += 16)
                {
        ...
        if (!this.field_71290_O)
        {
            ...
            for (int j = 0; j < this.worlds.size(); ++j)
            {
                ...
                if (worldserver != null)
                {
                    if (!p_71267_1_)
                    {
                        field_71306_a.info("Saving chunks for level \'" + worldserver.func_72912_H().func_76065_j() + "\'/" + worldserver.field_73011_w.func_80007_l());
                    }
    ...
    public void run()
    {
        ...
        catch (Throwable throwable)
        {
            ...
            if (crashreport.func_71508_a(file1))
            {
                field_71306_a.severe("This crash report has been saved to: " + file1.getAbsolutePath());
            }
            else
            {
